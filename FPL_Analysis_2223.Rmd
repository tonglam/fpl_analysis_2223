...
# new data table
knitr::kable(fpl_data[1:5,], caption = "First 5 rows of fpl_data adding per 90")
...
Let's begin by examining the distribution of actual per 90 values. The boxplot allows us to observe that there are outliers present in the actual values that deviate significantly from the others.
...
For observing the correlation between a player's cost and their performance, we can focus on regular players who have played more than 90 minutes and scored at least one goal or provided at least one assist for an entire season. These regular players are the foundation of every club's attacking performance. We can use a scatter plot with the `geom_smooth()` function to observe any possible correlation. For a small dataset, we will use *LOESS* method to fit the smooth curve. [[12]][ref12]

To prepare for visualization, it is necessary to perform some data transformations. Two new data frames can be created: one named "goals_performance" which combines the players' Goals90 and xG90, and another named "assists_performance" which combines the players' assist data. For defenders, a "goalsConceded_performance" can be created to observe their actual performance versus expected performance.
...
# Explore Club's Penalty Takers

Penalties are the most probable opportunities to score during a football match, with each penalty shoot given an xG of 0.78 xG of the StatsBomb xG 2022 model[[13]][ref13]. Accordingly, clubs should meticulously choose their penalty takers. Ideally, a player with a higher conversion ratio would be superior at finishing, possess improved shooting skills, and increase the likelihood of scoring the penalty kick.

## Penalty Takers Data Transform
...
# change numerical values to logical values
pk_taker_data <- fpl_data %>%
  select(c("Player", "Club", "Goals90", "xG90", "PK_Taker")) %>%
  mutate(PK_Taker = ifelse(is.na(PK_Taker), FALSE, TRUE))
# grouped by pk_taker, observe the goals performance
# use a FUN = mean here, because each group may contains different number of players
pk_taker_data <-
...
knitr::kable(pk_taker_data[1:8, ], caption =  "First 8 rows of pk_taker data")
```

## Performance of Club PK Takers

To visualize the performance of players who take penalty kicks and those who do not, you can use a stacked bar plot. A positive difference occurs when a player's actual value is greater than their expected value, indicating that they performed better than most other players. Conversely, a negative difference occurs when a player's actual value is less than their expected value, indicating that they performed worse than others. A higher diff value suggests that the player has better shooting technique and is more likely to score goals, as it indicates that the actual goals value is greater than the expected value.
...
[ref12]: https://www.itl.nist.gov/div898/handbook/pmd/section1/pmd144.htm
[ref13]: https://statsbomb.com/soccer-metrics/expected-goals-xg-explained/
