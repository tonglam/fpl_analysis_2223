...
ui <-  fluidPage(
  ...
  titlePanel("Total points distribution across different levels"),
...
server <-  function(input, output) {
  ...
  blox <- function(plot_data) {
    plot_data %>%
      ...
      geom_boxplot() +
      stat_summary(fun = mean, geom = "point", shape = 20, size = 10, color = "#ec7014", fill = "#ec7014") +
...
shinyApp(ui, server, options = list(height = '500', width = '100%'))
```
...
server <- function(input, output) {
  ...
  output$scatter <- renderPlot({
    ...
    ggplot(plot_data, aes(x = Points, y = Cost)) +
      ...
      labs(
        title = "Compare Player Points by Level or Value",
        x = "total points",
        y = "cost(m)"
      ) +
      theme(
...
server <- function(input, output, session) {
  ...
  output$table <- renderTable({
    radar_data_filter() %>% 
      mutate(Player = c(input$player1, input$player2), .before = Points)
  })
  
}


shinyApp(ui, server, options = list(height = '600', width = '100%'))
```
...
Outlier should be players play less than 90, so their per90 values are extremely high.
Transfer these data into NA, because when we are trying to analyse the performance of players by using the per90 data, these players who played less than 90 mins make no sense.
...
server <- function(input, output) {
  ...
  output$plot <- renderPlot({
    ...
    if (length(plot_list) > 0) {
      # add title
      title <- textGrob("Compare players' actual values and expected values", gp = gpar(fontsize = 15))
      # arrange
      grid.arrange(
        grobs = plot_list,
        ncol = 2,
...
shinyApp(ui, server, options = list(height = '700', width = '100%'))
```
...
heatmap_data <- plot_data %>%
  select(Points:xGc90_diff) %>%
  as.matrix()

# color, green for below average, blue for above average
color_1 <-
  colorRampPalette(colors = c('#5190FF', '#efedf5'))(6)[1:5]
color_2 <- colorRampPalette(colors = c('#efedf5', '#00FF87'))(6)

# classify attack stats and defence stats
col_side_color <-
  ifelse(
    attr(heatmap_data, which = "dimnames")[[2]] %in% c("CS90", "GC90", "xGc90", "xGc90_diff"),
    "#ec7014",
    "#737373"
  )
...
legend(
  "topleft",
  legend = c("GKP", "DEF", "MID", "FWD"),
  cex = 0.8,
  fill = c("#7fc97f", "#386cb0", "red", "#fdb462")
)

legend(
  "top",
  legend = c("Above average", "Average", "Below Average"),
  cex = 0.8,
  fill = c("#5190FF", "#efedf5", "#00FF87")
)

legend(
  "topright",
  legend = c("Offensive", "Defensive"),
  cex = 0.8,
  fill = c("#ec7014", "#737373")
)
