...
get_cost_level <- function(cost_bin) {
  levels <- levels(fpl_data$Cost_bin)
  case_when(
    cost_bin == levels[1] ~ "Low",
    cost_bin == levels[2] ~ "Mid",
    cost_bin == levels[3] ~ "Premium",
    .default = NA
  )
}
...
```{r xG, results='hide'}
# fpl_data %>% mutate(Cost_bin = cut_interval(fpl_data$Cost, 3)) %>% View()
