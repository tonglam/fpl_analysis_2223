...
To prepare for visualization, it is necessary to perform some data transformations. Two new data frames can be created: one named "goals_performance" which combines the players' Goals90 and xG90, and another named "assists_performance" which combines the players' assist data. For defenders, a "goalsConcede_performance" can be created to observe their actual performance versus expected performance.
...
Penalties are the most probable opportunities to score during a football match, with each penalty shoot given an xG of 0.78 xG of the StatsBomb xG 2022 model[[]][]. Accordingly, clubs should meticulously choose their penalty takers. Ideally, a player with a higher conversion ratio would be superior at finishing, possess improved shooting skills, and increase the likelihood of scoring the penalty kick.
...
To visualize the performance of players who take penalty kicks and those who do not, a stacked bar plot can be used. A higher diff value suggests that the player has better shooting technique and is more likely to score goals, as it indicates that the actual goals value is greater than the expected value.
...
col_side_color <-
  case_when(
    ...
    attr(heatmap_data, which = "dimnames")[[2]] %in% c("CS90",
                                                       "GC90",
                                                       "xGC90",
                                                       "xGC90_diff") ~ "#737373",
    .default = "purple"
