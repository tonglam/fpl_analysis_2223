...
model_data <- model_data %>%
  mutate(
    corners_and_indirect_freekicks_order = ifelse(is.na(corners_and_indirect_freekicks_order), FALSE, TRUE),
    penalties_order = ifelse(is.na(penalties_order), FALSE, TRUE),
    direct_freekicks_order = ifelse(is.na(direct_freekicks_order), FALSE, TRUE)
  )
...
model_data <- model_data %>%
  ...
  mutate(bonus_per_90 = bonus / minutes * 90, .after = bonus) %>%
...
model_data$goals_scored_per_90 <-
  ifelse(is.na(model_data$goals_scored_per_90), 0, model_data$goals_scored_per_90)
model_data$bouns_per_90 <-
  ifelse(is.na(model_data$bonus_per_90), 0, model_data$bonus_per_90)
model_data$red_cards_per_90 <-
  ifelse(is.na(model_data$red_cards_per_90), 0, model_data$red_cards_per_90)
model_data$threat_per_90 <-
  ifelse(is.na(model_data$threat_per_90), 0, model_data$threat_per_90)
model_data$influence_per_90 <-
  ifelse(is.na(model_data$influence_per_90), 0, model_data$influence_per_90)
model_data$creativity_per_90 <-
  ifelse(is.na(model_data$creativity_per_90), 0, model_data$creativity_per_90)
model_data$own_goals_per_90 <-
  ifelse(is.na(model_data$own_goals_per_90), 0, model_data$own_goals_per_90)
model_data$yellow_cards_per_90 <-
  ifelse(is.na(model_data$yellow_cards_per_90), 0, model_data$yellow_cards_per_90)
model_data$assists_per_90 <-
  ifelse(is.na(model_data$assists_per_90), 0, model_data$assists_per_90)
...
train <- model_data[fortrain,]
test <- model_data[!fortrain,]
#
```
...
features <- model_data[, c("Points", "Cost", "Minutes", "Goals", "xG", "Assists", "xA", "GC", "xGC", "CS")]
target <- fpl_data$Label
