...
catAUC2 <- ggplot(test_data) + geom_density(aes(x=pred_corners_and_indirect_freekicks_order, color=as.factor(player_type)))
...
pretty_perf_table <- function(model,train_data,test_data) {
  ...
  truth_train <- train_data[, outcome]
  pred_test <- predict(model, newdata=test_data)
  truth_test <- test_data[, outcome]
