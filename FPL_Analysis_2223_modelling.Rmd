...
roc_1 <- rocit(score=predcol1, class=outcol1==pos.label)
roc_2 <- rocit(score=predcol2, class=outcol2==pos.label)
plot(roc_1, col = c("lightblue","forestgreen"), lwd = 3,
legend = FALSE,YIndex = FALSE, values = TRUE, asp=1)
lines(roc_2$TPR ~ roc_2$FPR, lwd = 3,
col = c("pink","forestgreen"), asp=1)
legend("bottomright", col = c("lightblue","pink", "forestgreen"),
c("Test Data", "Training Data", "Null Model"), lwd = 2)
}
# 
# plot_roc(pred_test_roc, dTest[[target]],
# pred_train_roc, dTrain[[target]])
...
main_performance <- function(pred_train,pred_test,truth_train,truth_test, pos=pos.label, name="model") { #calculate the AUC
...
sep='')
tmodel1 <- rpart(player_type ~., tree_train_data_1,method = 'class')
rpart.plot(tmodel1)
...
predict_train <- as.numeric(predict(tmodel1, newdata=tree_train_data_1, type = "class"))
predict_test <- as.numeric(predict(tmodel1, newdata=tree_test_data_1, type = "class"))
truth_train <- ifelse(tree_train_data_1$player_type == "Offensive", 1, 2)
truth_test <- ifelse(tree_test_data_1$player_type == "Offensive", 1, 2)

tmodel_performance <- main_performance(predict_train,predict_test,tree_train_data_1$player_type,tree_test_data_1$player_type, name="tmodel")
...
tcm1 <- CM_metrics(predict_train, predict_test,truth_train, truth_test, "tmodel-Train", "tmodel-Test")
```
###For the combined variables - 2
- Build the model
...
tmodel2 <- rpart(player_type ~., data=tree_train_data_2, method = 'class')

predict <- as.numeric(predict(tmodel2, newdata=tree_train_data_2, type = "class"))

print(calcAUC(predict, tree_train_data_2[, target]))

predict <- as.numeric(predict(tmodel2, newdata=tree_test_data_2, type = "class"))
...
predict_train <- as.numeric(predict(tmodel2, newdata=tree_train_data_2, type = "class"))
print(calcAUC(predict_train, train_data[, target]))

predict_test <- as.numeric(predict(tmodel2, newdata=tree_test_data_2, type = "class"))
...
pred_train_roc <- predict(tmodel1, newdata=tree_train_data_1)
pred_test_roc <- predict(tmodel1, newdata=tree_test_data_1)

plot_roc(pred_test_roc, tree_test_data_1[[target]],
pred_train_roc, tree_train_data_1[[target]])
```
...
input <- as.matrix(xgb_train_data_1[-c(1, 2)])
...
test_input <- as.matrix(xgb_test_data_1[-c(1, 2)])

pred <- predict(model, test_input)

print(calcAUC(pred, xgb_test_data_1[, target]))
...
input <- as.matrix(xgb_train_data_2[-c(1, 2)])
...
test_input <- as.matrix(xgb_test_data_2[-c(1, 2)])

pred <- predict(model, test_input)

print(calcAUC(pred, xgb_test_data_2[, target]))
