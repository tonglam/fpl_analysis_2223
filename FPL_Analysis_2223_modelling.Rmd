...
```
- Evaluate the performance by loglikelihood
```{r}
# Compute the likelihood of the Null model on the test data
logNull <- logLikelihood(sum(test_data[,outcome]==pos.label)/nrow(test_data), test_data[,outcome]==pos.label)
...
```{r Single variable predictions: categorical}
...
for(v in catVars) {
  ...
  train_data[,pi] <- mkPredC(train_data[,'player_type'], train_data[,v], train_data[,v])
  test_data[,pi] <- mkPredC(train_data[,'player_type'], train_data[,v], test_data[,v])
}
```

- Evaluate the categorical performance by AUC

```{r AUC calculate function}
...
for(v in catVars) {
  ...
  aucTrain <- calcAUC(train_data[, pi], train_data[, "player_type"])
    ...
    print(sprintf(
      "%s: trainAUC: %4.3f; TestAUC: %4.3f",
      pi, aucTrain, aucTest
    ))
}
```
- further explore the AUC values of two example categorical columns
...
```{r Single variable predictions: numerical}
...
result_tribble <- tribble(~ pred, ~ trainAUC, ~ testAUC)
for (v in numericVars) {
  ...
  if (aucTrain >= 0.4) {
    ...
    result <- sprintf("%s: trainAUC: %4.3f; TestAUC: %4.3f",
                      pi, aucTrain, aucTest)
    result_tribble <- add_row(result_tribble, pred = result, trainAUC = aucTrain, testAUC = aucTest)
  }
}
# Sort the result based on aucTrain value
result_tribble <- arrange(result_tribble, desc(trainAUC), desc(testAUC))
result_tribble
```
- further explore the AUC values of two example numerical columns
...
for (v in catVars) {
  pi <- paste('pred_', v, sep='')
  devDrop <- 2*(logLikelihood(test_data[,pi], test_data[,outcome]==pos.label) - logNull)
  if (devDrop >= minDrop) {
