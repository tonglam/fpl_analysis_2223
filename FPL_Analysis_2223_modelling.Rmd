...
trainPredictions <-
  predict(tmodel_1, newdata = tree_train_data_1, type = "class")
print(calcAUC(as.numeric(trainPredictions), tree_train_data_1[, target]))
trainAccuracy <-
  mean(trainPredictions == tree_train_data_1$player_type)
print(paste("Training Accuracy:", trainAccuracy))

testPredictions <-
  predict(tmodel_1, newdata = tree_test_data_1, type = "class")
print(calcAUC(as.numeric(testPredictions), tree_test_data_1[, target]))
testAccuracy <-
  mean(testPredictions == tree_test_data_1$player_type)
...
predict_train <- as.numeric(predict(tmodel_1, newdata=tree_train_data_1, type = "class"))
predict_test <- as.numeric(predict(tmodel_1, newdata=tree_test_data_1, type = "class"))
...
tmodel_2 <- rpart(player_type ~., data=tree_train_data_2, method = 'class')

predict <- as.numeric(predict(tmodel_2, newdata=tree_train_data_2, type = "class"))

print(calcAUC(predict, tree_train_data_2[, target]))

predict <- as.numeric(predict(tmodel_2, newdata=tree_test_data_2, type = "class"))
...
predict_train <- as.numeric(predict(tmodel_2, newdata=tree_train_data_2, type = "class"))
print(calcAUC(predict_train, train_data[, target]))

predict_test <- as.numeric(predict(tmodel_2, newdata=tree_test_data_2, type = "class"))
...
plot_roc <- function(predcol1, outcol1, predcol2, outcol2) {
  ...
  lines(
    roc_2$TPR ~ roc_2$FPR,
    lwd = 3,
    col = c("pink", "forestgreen"),
    asp = 1
  )
  legend(
    "bottomright",
    col = c("lightblue", "pink", "forestgreen"),
    c("Test Data", "Training Data", "Null Model"),
    lwd = 2
  )
}
pred_train_roc <- predict(tmodel_1, newdata = tree_train_data_1)
pred_test_roc <- predict(tmodel_1, newdata = tree_test_data_1)

plot_roc(pred_test_roc[, 1],
