...
catResult <- tribble(~ pred, ~ trainAUC, ~ testAUC)
...
numResult <- tribble(~ pred, ~ trainAUC, ~ testAUC)
...
selCatResult <- tribble( ~ pred, ~ deviance)
...
selCatResult <- arrange(selCatResult, desc(deviance))
selCatResult
```
...
# store the top performing categorical variables.
selNumResult <- tribble( ~ pred, ~ deviance)
observe_selNumResult <- tribble( ~ pred, ~ deviance)
...
for (v in numericVars) {
  ...
  if (devDrop >= minDrop) {
    result <- sprintf("%6s, deviance reduction: %g", v, devDrop)
    selNumResult <-
      add_row(selNumResult,
              pred = result,
              deviance = devDrop)
    if (v %in% observeVars) {
      observe_selNumResult <- add_row(observe_selNumResult,
                                       pred = result,
                                       deviance = devDrop)
    }
  }
...
# sort by deviance desc
selNumResult <- arrange(selNumResult, desc(deviance))
selNumResult
observe_selNumResult <- arrange(observe_selNumResult, desc(deviance))
observe_selNumResult

# drop actual variable
numericVars = numericVars[!numericVars %in% observeVars]
numericVars
```
