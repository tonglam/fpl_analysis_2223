...
convert_order_to_name <- function(value) {
  case_when(
            value == 1 ~ "First",
            value == 2 ~ "Second",
            value == 3 ~ "Thrid",
...
model_data <- model_data %>%
  mutate(
    corners_and_indirect_freekicks_order = convert_order_to_name(corners_and_indirect_freekicks_order),
    penalties_order = convert_order_to_name(penalties_order),
    direct_freekicks_order = convert_order_to_name(direct_freekicks_order)
  )
...
missing_values <- apply(is.na(train_data), 2, sum)
...
train_data <- train_data %>%
  ...
  mutate(
    player_type = ifelse(position == 1 |
                           position == 2, 0, 1),
    .before = position
  ) %>%
  select(-position)

knitr::kable(train_data[1:5, 1:5], caption =  "First 5 rows and first 5 conlumns of data")
```
