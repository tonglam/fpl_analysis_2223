...
3 columns have missing values by checking. We decided to convert these columns to logical values, True represents not missing and False represents missing. Then, we check the missing values again, and there are no missing values in the dataset.
...
With the above transformation, we removed the some features.
...
## Add Target Value
There are 4 position, we take "GKP" and "DEF" as a defensive player, and take "MID" and "FWD" as a offensive player. So "Offensive" and "Defensive" are our classes. Then we create a new column called "player_type_value" to represent the player type with numerical values, 1 represents offensive player, 0 represents defensive player.
...
### plot target value
To easily see the quantity and distribution of the binary value, we plot a barplot and a donut plot to show it.

```{r preprocess_target_barplot}
#define plot function - barplot
...
#define plot function - donut plot
...
## Read Data
Import the processed data and plot the binary value.
...
# knitr::kable()
```
From these 2 plots, we can see that the data is balanced, because the difference between the offensive players and defensive players are not that much. Next we do some sampling for the data.
...
As mentioned before, offensive and defensive are our binary problem. We will use the following models to predict the target value.

## Single Variable Model Build
With the previous split data set, we built a single-variable model to see the performance of each variable.
### Categorical
- Build model 
```{r categorical_predictions}
#define a predict function for categorical variables
...
# call the function for the candidate columns
for (v in catVars) {
...
# sort the features by train AUC desc, then test AUC desc
catResult <- arrange(catResult, desc(trainAUC), desc(testAUC))
catResult
```
As we can see from the result, the best AUC for training and testing data are 0.55 and 0.61 which is from "penalties_order" and good enough.

- Plotting the density plot for predicted probabilities
...
```
Through the density charts above, we can check the relationship between predicted probabilities and the variables. The higher the predicted probabilities, the more likely the player is offensive.

- Plotting the ROC

```{r ROC categorical}
# define a roc plotting function for single variable model
plot_roc_sa <-
  function(predcols,
           outcols,
           colors = c("salmon", "lightblue", "lemonchiffon", "seagreen","orchid","lightpink"),
           overlaid = F,
           features,
           title) {
    ...
    par(new = overlaid)
    ROCit_obj <- rocit(score = predcols[[i]], class = outcols[[i]] == pos.label)
    plot(
      ROCit_obj,
      col = c(colors[i], 1),
      legend = FALSE,
...
plot_roc_sa(
  predcols = list(
    test_data$pred_penalties_order,
    test_data$pred_corners_and_indirect_freekicks_order,
    test_data$pred_direct_freekicks_order
  ),
  outcols = list(test_data[, target], test_data[, target], test_data[, target]),
  features = c("pred_penalties_order", "pred_corners_and_indirect_freekicks_order", "pred_direct_freekicks_order"),
  title = "ROC for single variable (Categorical)",
  overlaid = TRUE
)

``` 
From the ROC, We also can see the 3 features perform almost the same. The highest one is "pred_penalties_order".
...
#define a predict function for numerical variables
...
numResult['pred']
```
From this table, we can see the best AUC for training and testing data are 0.76 and 0.80 which is pretty good. And the following features still have good performance and each score is really close. Moreover, the auc of the top ten are all above 0.6.


- Plotting the density plot for AUC values
...
#according to the above auc performance, choose the top 6 features and plot
for (col in Ncol[1:6]) {
...
```
These features still show good results in the density diagram.
...
plot_roc_sa(
  predcols = list(
    test_data$pred_expected_goal_involvements_per_90,
    test_data$pred_threat_per_90,
    test_data$pred_expected_goals_per_90,
    test_data$pred_creativity_per_90,
    test_data$pred_expected_assists_per_90,
    test_data$pred_starts_per_90
  ),
  outcols = list(test_data[, target], test_data[, target], test_data[, target], test_data[, target], test_data[, target], test_data[, target]),
  features = c("pred_expected_goal_involvements_per_90", "pred_threat_per_90", "pred_expected_goals_per_90","pred_creativity_per_90","pred_expected_assists_per_90","pred_starts_per_90"),
  title = "ROC for single variable(Numerical)",
  overlaid = TRUE
)

```
As we can see, most of the features are very close to the Y-axis and the X axis are relatively small. It means the top 6 features are performing very well. The highest one is "pred_expected_goal_involvements_per_90".

- Conclusion:
The results indicate that the numerical features perform exceptionally well, suggesting that the prediction accuracy is significantly high.
...
- Null Model
```{r log_likelihood}
# define compute function - log likelihood
...
for (v in catVars) {
  ...
  if (devselect >= minDrop) {
    result <- sprintf("%6s, deviance reduction: %g", v, devselect)
    select_cat_result <-
      add_row(
        select_cat_result,
        feature = v,
...
# sort by deviance ASC
select_cat_result <- arrange(select_cat_result, deviance)
select_cat_result
```
The results show that penalties_order has the lowest deviance(10.30) in the categorical features, which means this feature is highly correlated with the target. It still has the highest AUC score (train0.56/test0.61).
...
# sort by deviance ASC
sel_num_result <- arrange(sel_num_result, deviance)
observe_selNumResult <-
  arrange(observe_selNumResult, deviance)
...
```
The results show that pred_expected_goal_involvements_per_90 has the lowest deviance(3.27) in the numerical features, which still has the highest AUC score (train0.76/test0.80), which means this feature is highly correlated with the target. Meanwhile, the following numerical features still perform very low scores and high AUC scores. 
...
From the previous section, we have identified the following features that have good performance in predicting the target variable. Then we use the first method to combine these features.
...
According to the multiple usage, we define a function so that we can reuse it to do multiple performance calculations. This function includes several main methods, such as confusion matrix, AUC, and likelihood. It will be used in the following sections and increase the efficiency of the code. At the same time, it is convenient to compare the results of each model.
...
single_performance <- function(pred, truth, name = "model") {
  ...
  if (str_detect(name, "xgb")) {
    ...
    truth_class <- ifelse(truth > 0.5, "Offensive", "Defensive")
  }
  factor_pred <- pred
...
```
From this table, firstly we can see that the performance of the two tree models are very similar. The difference of 2 models between train and test in the first 5 metrics are small, which around 0.02, it indicates 2 models are fit. 
For the tree models, we also can see that Accuracy are more than 0.75, which indicates that can successfully accurate classification to predict the player type. Precision are more than 0.66 that seems fair enough. Recall have really highest score in all metrics, The high recall rate indicates that these tree models successfully captures most real positive samples.Tree models achieved a good balance between accuracy and recall with high F1 score(each > 0.76). AUC still good enough.
...
```
For tree model 1, the ROC curve is not very good. The AUC is 0.76/0.78(train/test), which is good. The ROC curve is not very close to the top left corner. However, there is little difference between the training data and the test data curve, which shows that the training data model has high accuracy.
...
```
For tree model 2, the ROC curve performe not very well and the AUC is 0.77/0.79(train/test), which is a little bit better than tree model 1. The curve is not very close to the top left corner as well. Like other metrics that mentioned before, the two models behave very closely.
Next, we visualise the decision tree for the two models.
...
Next, we keep the same parameters and build another XGBoost model with features selected by RFE.
...
xgb_pref
```
The above table shows the performance of the two XGBoost models. Among all metrics, the disparity between their scores is minimal. Both models perform the highest recall and AUC, which are remarkably similar and close to 1. It suggests that the 2 models have excellent performance in this binary classification, but their adaptability and performance in other measures should be further assessed. The other remaining metrics (Accuracy, Precision, F1 score), all above 0.67, mostly about 0.76, which also shows that 2 models are effective. And log likelihoods are much better than NUll model.
Overall, the performance of XGBoost 2 is slightly better than XGBoost 1. Then, We will use ROC to further evaluate the model in the next section.
...
```
From this plot, we can see that the ROC curves are extremely close to Y axis and top left corner, which means that the model has a high true positive rate and a low false positive rate.
...
```
For XGBoost 2, the ROC curves are also perform true positive rate and a low false positive rate like XGBoost 1.



## Models Evaluation

## LIME for Explaining Models
...
```
We can see from the multiple plots and table that 结论.
